...
exports.findData = async (req, res) => {
  const cat = await categoryModel.findAll();
  categoryModel.hasMany(portModel, { foreignKey: "id" });
  portModel.belongsTo(categoryModel, { foreignKey: "category_id" });
  const users = await portModel.findAll({ include: [categoryModel] });
  res.send(users);
};
